...
class LocalIndexer:
    ...
    def __init__(self, settings: Settings, sandbox: WorkspaceSandbox, store: SessionStore, embeddings: Any = None) -> None:
        self.settings, self.sandbox, self.store, self.embeddings = settings, sandbox, store, embeddings
    ...
    async def refresh(self) -> dict[str, int | bool]:
        ...
        if self.embeddings:
            for candidate in candidates[:self.settings.max_files_indexed]:
                try:
                    display_path = self.sandbox.display(candidate)
                    await self.embeddings.index_file(display_path)
                except Exception:
                    continue

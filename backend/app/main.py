...
class Runtime:
    def __init__(self, settings: Settings) -> None:
        ...
        self.limiter = OllamaRateLimiter(settings)
        ...
        self.embeddings = LocalEmbeddings(settings, WorkspaceSandbox(settings), self.store, self.ollama)
        self.tools = ToolRegistry(settings, embeddings=self.embeddings)
...
async def refresh_index() -> dict[str, Any]:
    ...
    return await LocalIndexer(runtime().settings, runtime().tools.sandbox, runtime().store, embeddings=runtime().embeddings).refresh()

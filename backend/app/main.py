...
async def export_audit(session_id: str | None = None, query: str = "", format: str = "json"):
    entries = runtime().store.audit_entries(session_id, query)
    if format.lower() == "csv":
        import csv
        import io
        ...
        writer.writerow(["id", "timestamp", "session_id", "actor", "action_type", "tool_name", "target", "result", "dry_run", "approved_by"])
        for entry in entries:
            payload = entry.get("payload_json") or {}
            writer.writerow([
                entry.get("id"),
                entry.get("created_at") or payload.get("timestamp"),
        ...
        return Response(content=output.getvalue(), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=audit_export.csv"})

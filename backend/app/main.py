...
class Runtime:
    def __init__(self, settings: Settings) -> None:
        ...
        self.embeddings = OllamaEmbeddings(self.ollama)
...
async def semantic_search(query: str, path: str = ".", max_results: int = 10, model: str | None = None) -> dict[str, Any]:
    try:
        return await runtime().tools.search.semantic_search(query, embeddings=runtime().embeddings, path=path, max_results=max_results, embedding_model=model)
    except Exception as exc:
        raise HTTPException(400, str(exc)) from exc

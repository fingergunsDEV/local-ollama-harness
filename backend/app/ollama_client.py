...
class OllamaClient:
    ...
    async def embeddings(self, model: str, prompt: str) -> list[float]:
        """Generate vector embeddings for a given text prompt using Ollama local daemon."""
        payload = {"model": model, "prompt": prompt}
        slot = await self.limiter.generation_slot()
        async with slot:
            try:
                response = await self.http.post("/api/embeddings", json=payload)
                response.raise_for_status()
                data = response.json()
                return list(data.get("embedding", []))
            except httpx.HTTPError as exc:
                raise OllamaUnavailable("Ollama embeddings failed") from exc

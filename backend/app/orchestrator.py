...
class AgentOrchestrator:
    ...
    async def run(self, session_id: str, user_content: str) -> AsyncIterator[dict[str, Any]]:
        ...
        try:
            while True:
                ...
                final: dict[str, Any] = {}
                response_text = ""
                ctx_len = capabilities.get("context_length") if isinstance(capabilities, dict) else None
                max_chars = min(int(ctx_len) * 3, 100_000) if ctx_len and isinstance(ctx_len, (int, float)) and ctx_len > 0 else 48_000
                async for event in self.client.chat(session["model"], self._fit_context(messages, max_chars=max_chars), TOOL_SCHEMAS if native_tools else None):
                    budget.check_time()
                    if event["type"] == "token":
    ...
    def _snapshot_if_file(self, session_id: str, name: str, args: dict[str, Any]) -> str | None:
        if name in {"write_file", "delete_file", "move_file"}: return self.tools.snapshot(session_id, args.get("path") or args.get("source"))
        return None

    def _fit_context(self, messages: list[dict[str, Any]], max_chars: int = 48_000) -> list[dict[str, Any]]:
        # Conservative character-based truncation protects the system message when model context
        # metadata is unknown. Production capability detection may adjust this per-model budget.
    ...
    def _parse_react_action(text: str) -> dict[str, Any]:
        ...
        # Attempt to find JSON object bounds if direct parse fails
        try:

...
class AgentOrchestrator:
    ...
    async def _invoke(self, session: dict[str, Any], message_id: str, name: str, args: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
        ...
        try:
            ...
            if action_type:
                preview = await self.tools.execute(name, args, dry_run=True)
                ...
                if session["dry_run"]:
                    ...
                    if not decision.approved:
                        ...
                        result = await self.tools.execute(name, args, dry_run=False); approved_by = "user"
                        if snapshot: result["snapshot"] = snapshot
                else:
                    snapshot = self._snapshot_if_file(session["id"], name, args)
                    result = await self.tools.execute(name, args, dry_run=False); approved_by = "auto"
                    if snapshot: result["snapshot"] = snapshot
            else:
                result = await self.tools.execute(name, args, dry_run=False); approved_by = "n/a"

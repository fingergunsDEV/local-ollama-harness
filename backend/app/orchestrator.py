...
class AgentOrchestrator:
    ...
    @staticmethod
    def _validate_react_data(data: Any) -> dict[str, Any]:
    ...
    def _parse_react_action(text: str) -> dict[str, Any]:
        ...
        try:
            ...
            res = AgentOrchestrator._validate_react_data(data)
            if not res.get("invalid"):
                return res
        except json.JSONDecodeError:
            pass

        # 2. Markdown code block extraction (e.g., ```json ... ``` or ``` ... ```)
        code_blocks = re.findall(r"```(?:json)?\s*(.*?)\s*```", cleaned, re.DOTALL | re.IGNORECASE)
        ...
        # 3. Structural JSON object candidate extraction {...}

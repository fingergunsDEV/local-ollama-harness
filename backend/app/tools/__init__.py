...
TOOL_SCHEMAS = [
 ...
 {"type":"function","function":{"name":"move_file","description":"Move or rename a regular file inside the workspace; returns a plan in dry-run mode.","parameters":{"type":"object","properties":{"source":{"type":"string"},"destination":{"type":"string"}},"required":["source","destination"],"additionalProperties":False}}},
 {"type":"function","function":{"name":"copy_file","description":"Copy a regular file inside the workspace; returns a plan in dry-run mode.","parameters":{"type":"object","properties":{"source":{"type":"string"},"destination":{"type":"string"}},"required":["source","destination"],"additionalProperties":False}}},
...
class ToolRegistry:
    def __init__(self, settings: Settings) -> None:
        ...
        self.search = SearchTools(settings, self.sandbox)
        ...
        self._handlers: dict[str, Callable[..., dict[str, Any]]] = {
            ...
            "keyword_search": self.search.keyword_search, "write_file": self.files.write_file,
            "create_file": self.files.create_file, "delete_file": self.files.delete_file,
            "move_file": self.files.move_file, "copy_file": self.files.copy_file,
            "git": self.git.execute, "exec": self.exec.execute,
        }

    @staticmethod
    def action_type(name: str, args: dict[str, Any]) -> str | None:
        if name in {"write_file", "create_file", "move_file", "copy_file"}: return "write"
    ...
    def execute(self, name: str, args: dict[str, Any], dry_run: bool) -> dict[str, Any]:
        if name not in self._handlers: raise ValueError(f"unknown tool: {name}")
        handler = self._handlers[name]
        if name in {"write_file", "create_file", "delete_file", "move_file", "copy_file", "git", "exec"}:

...
def test_parse_react_action_with_markdown_codeblock():
    markdown_json = "```json\n{\"action\": \"keyword_search\", \"arguments\": {\"query\": \"test\"}}\n```"
    parsed = AgentOrchestrator._parse_react_action(markdown_json)
    assert parsed["action"] == "keyword_search"
    assert parsed["arguments"] == {"query": "test"}

    final_markdown = "```\n{\"final\": \"Task complete\"}\n```"
    parsed_final = AgentOrchestrator._parse_react_action(final_markdown)
    assert parsed_final["final"] == "Task complete"
